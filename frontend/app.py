...
# FastAPI Http post 호출 로직 필요 시 사용(2026.07.22 minjae)
...
# Streamlit Cloud 배포 call_backend 함수 로직 구현(2026.07.22 minjae)
def call_backend(prompt: str, history: list, retriever_type: str):
    """
    같은 프로세스 안에서 파이프라인을 직접 호출한다.

    로컬에서는 FastAPI에 HTTP로 붙었지만, Streamlit Cloud는 단일 프로세스만
    띄우므로 백엔드 서버를 별도로 못 올린다. 그래서 함수를 직접 호출한다.
    반환 계약(dict, error)은 그대로라 UI 렌더링 코드는 손대지 않는다.
    ...
    try:
        data = get_ai_response(
            query=prompt, history=history, retriever_type=retriever_type,
        )
        return data, None
    except RateLimitError:
        return None, (
            "OpenAI 크레딧이 소진되었습니다. Manage app → Settings → Secrets에서 "
            "OPENAI_API_KEY를 확인하세요."
        )
    except AuthenticationError:
        return None, "OpenAI 키가 유효하지 않습니다. Secrets의 OPENAI_API_KEY를 확인하세요."
    except Exception as e:
        return None, f"파이프라인 실행 중 오류: {type(e).__name__}: {str(e)[:300]}"
